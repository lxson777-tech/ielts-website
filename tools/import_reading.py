...
import (source numbers 319, 318, 317, 316, 313 down to 298, renumbered to the
student-facing reading-full-001.ts through reading-full-020.ts via
LOCAL_TO_SOURCE):
...
def load_existing_tests(min_number: int = 1) -> list[tuple[str, dict]]:
    """Load every already-imported reading-full-NNN.ts (NNN >= min_number) as JSON,
    for duplicate-passage checks. Every reading test on the site is now imported
    (the five hand-written in-house tests were removed on 2026-09-13 and
    reading-full-001.ts through 020.ts are the renumbered original import, in
    the same single-JSON-literal format as everything after them), so there is
    no format gap left to skip; min_number only exists so a caller can narrow
    the comparison set if it ever needs to."""
    tests = []
...
def write_test(test: dict, path: Path) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Source test numbers to try, e.g. '297-278' (a descending range) or a "
             ...
             "selection (319, 318, 317, 316, 313 down to 298), renumbered to local "
             "tests 1-20 via LOCAL_TO_SOURCE.",
    )
    parser.add_argument(
        "--start",
        type=int,
        default=21,
        help="First output index for src/data/tests/reading-full-<NNN>.ts when "
             "--numbers is given (default 21, the first slot after the original 20).",
    )
    parser.add_argument(
        "--target",
        type=int,
        ...
        help="How many complete, non-duplicate tests to import before stopping. "
             "Defaults to the count of --numbers given.",
    )
    args = parser.parse_args()

    if args.numbers is None:
        # The original 20-test import: each source page keeps its explicit,
        # hand-checked local number from LOCAL_TO_SOURCE (students see "Test 1"
        # through "Test 20", not the publisher's page numbers), and any
        # hand-written explanation/evidence notes already on disk are carried
        # over rather than wiped. No duplicate check: this regenerates known
        # content, not new tests, so comparing it against itself would always
        # "find" a duplicate.
        manifest = {"selected": NUMBERS, "absentVerified": [315, 314], "skippedDuplicates": [], "tests": []}
    ...
    # Targeted range import (e.g. --numbers 297-278 --start 21): general
    # purpose, with duplicate-passage detection against every test already on
    # the site, and sequential local numbering starting at --start so each new
    # test's id/title (reading-full-<NNN>, "Academic Reading Test <NNN>")
    # matches the file it's written to.
    candidates = parse_number_spec(args.numbers)
    target = args.target or len(candidates)
    known_tests = load_existing_tests()
    manifest = {"selected": [], "absentVerified": [], "skippedDuplicates": [], "tests": []}

    # If the given numbers run out before we've collected `target` tests, keep
    # counting down past the end of the range (same step direction as the
    # range) rather than stopping short.
    extend_step = None
    if candidates:
    ...
    while imported < target:
        ...
        index += 1
        try:
            test = build(number, output_index)
