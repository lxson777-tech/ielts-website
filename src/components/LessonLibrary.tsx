...
import { SKILLS, type Skill } from '../data/lessons';
import { withBase } from '../lib/url';
import { isLessonComplete } from '../lib/progress';
import { libraryEntries, filterLibrary } from '../lib/lesson-library';
...
export default function LessonLibrary() {
  ...
  /* Search reads what the card actually shows, so a Russian phrase finds a
     Russian card, plus the English original. */
  const visible = useMemo(
    () => filterLibrary(entries, filter as Skill | 'all', query, (lesson) => `${lesson.title} ${lesson.description} ${t(lesson.title)} ${lesson.description ? t(lesson.description) : ''}`),
    [entries, filter, query, t],
  );
  return <div className="library-space">
    <div className="library-heading"><div><h1>{t('Learn at your pace.')}</h1><p>{t('Choose a skill, open a lesson, and mark it complete when you are ready to move on.')}</p></div><div className="library-count"><strong>{Math.min(limit, visible.length)} / {visible.length}</strong><span>{tn(Math.min(limit, visible.length), { one: 'lesson shown', other: 'lessons shown' })}</span></div></div>
    ...
    <div className="library-grid">{visible.slice(0, limit).map((lesson) => { const complete = mounted && isLessonComplete(lesson.key); const skillLabel = SKILLS.find((skill) => skill.id === lesson.skill)?.label; return <a className={`library-card skill-${lesson.skill}`} key={lesson.key} href={withBase(lesson.href)}><div className="library-card-top"><span>{skillLabel ? t(skillLabel) : ''}{lesson.overview ? ` · ${t('Overview')}` : ''}</span>{complete && <span className="library-complete">✓ {t('Complete', undefined, 'status')}</span>}</div>{lesson.moduleLabel && <span className="library-card-module">{t(lesson.moduleLabel)}</span>}<h2>{t(lesson.title)}</h2><p>{lesson.description ? t(lesson.description) : t('Build your {skill} skills with this focused lesson.', { skill: (skillLabel ? t(skillLabel) : '').toLowerCase() })}</p><div className="library-card-link"><span>{t('Open lesson')}</span>{typeof lesson.minutes === 'number' && <span className="library-card-minutes">{t('{n} min', { n: lesson.minutes })}</span>}</div></a>; })}</div>
    {visible.length > limit && <button type="button" className="library-more" onClick={() => setLimit((n) => n + 12)}>{t('Show more lessons')}</button>}
