...
| R7.26-recall-and-use | 7 | Review includes recall and use in a sentence, not only recognising definitions. | WP21 | done | `contracts/catalog.ts` `VocabularySubskill` (`recognise-meaning`, `recall-from-meaning`, `use-in-a-sentence`, `collocation`, `topic-breadth`), spot-checked present. |
