...
2. Three things must be settled before any paywall code is written, because each one can invalidate the product. Whether we are allowed to charge for the imported exam bank, where Kazakh students' personal data may legally live, and whether a Kazakh buyer must be given a fiscal receipt. Each is a gate with an owner and a date in phase A, not a risk note.
...
- **Phase A, gates and paperwork, week one, no build days.** Register the ИП or ТОО. Apply to Paddle and to Halyk or Kaspi. Get the exam bank licensing position in writing for every imported test. Brief the lawyer (four documents, Russian and Kazakh) and the accountant (fiscal receipts, foreign currency payouts from Paddle, contract registration). Ask Halyk and the .kz registrar in writing whether a .com domain is acceptable and what the hosting location rule means for a static site, because if .kz is required the hosting choice has to be revisited. **Run a paid pre-sale**: a one page offer to the centre's own students at a founding price, paid by Kaspi Business invoice, access granted by hand. If twenty people will not pay, the packaging is wrong and the build changes shape.
...
3. **Paddle may decline us.** An education platform from a new Kazakh ИП selling access to third party exam material is the profile that gets extra verification. The fallback is named in section 4.
...
- [ ] Get the exam bank licensing position in writing before any paywall work starts.
